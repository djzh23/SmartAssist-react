...
export default function OverviewPage() {
  ...
  const shortHint = hint.length > 88 ? `${hint.slice(0, 85)}...` : hint
  ...
  return (
    <div id="app-uebersicht-start" className="relative min-h-0 flex-1 overflow-y-auto bg-transparent">
      ...
      <div className="relative z-10 mx-auto max-w-[1360px] space-y-5 px-4 py-4 sm:px-6 sm:py-6">
        <header className="overflow-hidden rounded-3xl border border-amber-300/20 bg-gradient-to-r from-[#21140b]/95 via-[#29190d]/95 to-[#1f140c]/95 p-4 shadow-landing-lg sm:p-6">
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div className="flex min-w-0 flex-1 gap-3">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-amber-300/25 bg-amber-500/14 text-amber-200 shadow-[0_0_18px_-8px_rgba(217,119,6,0.65)]">
                <LayoutDashboard className="h-5 w-5" aria-hidden />
              </div>
              <div className="min-w-0">
                <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-amber-200/75">Dein nächster Schritt</p>
                <h1 className="mt-0.5 text-2xl font-bold tracking-tight text-amber-50 sm:text-[2rem]">
                  Dein nächster Schritt
                </h1>
                <p className="mt-1.5 max-w-2xl text-sm text-amber-100/80">
                  {nextStepLabel}
                </p>
                <p className="mt-2 inline-flex max-w-full items-center gap-1.5 rounded-full border border-amber-300/25 bg-amber-400/10 px-2.5 py-1 text-[11px] font-medium text-amber-100/85">
                  <Lightbulb size={13} className="text-amber-200" />
                  <span className="truncate">KI Insight: {shortHint}</span>
                </p>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-2 md:justify-end">
              <Link
                to={heroCtaTo}
                className="inline-flex items-center gap-1.5 rounded-xl bg-primary px-3.5 py-2 text-sm font-semibold text-white shadow-lg shadow-amber-700/20 transition hover:bg-primary-hover"
              >
              ...
              <Link
                to="/applications"
                className="inline-flex items-center gap-1.5 rounded-xl border border-amber-200/25 bg-white/5 px-3.5 py-2 text-sm font-semibold text-amber-100 transition hover:bg-white/10"
              >
                Zur Pipeline
        ...
        {/* Centerpiece + compact right rail */}
        <section className="grid gap-4 xl:grid-cols-[minmax(0,2.45fr)_minmax(360px,1fr)]">
          <div className="rounded-3xl border border-amber-200/10 bg-[#1b120d]/85 p-4 shadow-landing-md sm:p-5">
          ...
          <div className="space-y-4">
            <div className="rounded-3xl border border-amber-200/10 bg-[#1b120d]/85 p-4 shadow-landing-md">
              ...
              {loading ? (
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  {metrics.map(row => {
                    const Icon = row.icon
                    return (
                      <div key={row.label} className="rounded-xl border border-amber-100/10 px-3 py-2.5">
        ...
        <section className="grid gap-4 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,2.05fr)]">
          <section className="rounded-3xl border border-amber-200/10 bg-[#1b120d]/85 p-4 shadow-landing-md">
            ...
            <div className="space-y-2">
              {activitySignals.map(item => {
                ...
                return (
                  <div
                    key={item.label}
                    className="flex items-center gap-2 rounded-xl border border-amber-100/10 px-3 py-2"
                  >
          ...
          <section className="rounded-3xl border border-amber-200/10 bg-[#1b120d]/85 p-4 shadow-landing-md">
            ...
            <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
              {WORKSPACE_LINKS.slice(0, 6).map(({ to, label, icon: Icon }) => (
                <li key={to}>
                  <Link
                    to={to}
                    className="group flex min-h-[3.7rem] items-center gap-3 rounded-xl border border-amber-100/10 px-3 py-2.5 transition duration-200 hover:border-amber-200/25 hover:bg-[#2a1c15]/40"
                  >
                    <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-amber-500/14 text-amber-200">
