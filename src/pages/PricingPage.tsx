...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <button
      ...
      className="w-full rounded-2xl border border-stone-600/40 bg-app-surface/90 px-5 py-4 text-left shadow-landing backdrop-blur transition-all duration-200 hover:shadow-landing-md"
    >
      <div className="flex items-center justify-between gap-3">
        <span className="text-sm font-semibold text-stone-100">{q}</span>
        {open
          ? <ChevronUp size={16} className="flex-shrink-0 text-stone-500" />
          : <ChevronDown size={16} className="flex-shrink-0 text-stone-500" />}
      </div>
      {open && (
        <p className="mt-2 text-sm leading-relaxed text-stone-400">{a}</p>
      )}
    </button>
  )
}
  ...
  if (!isLoaded) {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center bg-transparent">
        <Clock size={22} className="animate-pulse text-stone-500" />
      </div>
    )
  }
        ...
        <div className="mb-14 grid grid-cols-1 gap-5 md:grid-cols-3 md:items-start">
          {PLANS.map(plan => (
            <div
              key={plan.id}
              className={[
                'relative overflow-hidden rounded-3xl border-2 bg-app-surface/95 shadow-landing backdrop-blur transition-all duration-200',
                plan.accentBorder,
                plan.scale ? 'md:scale-[1.03] md:shadow-landing-promo' : '',
              ].join(' ')}
            >
              <div
              ...
              <div className={`relative px-5 pt-5 pb-4 ${plan.accentHeader}`}>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-stone-600/40 bg-app-raised/90">
                    {plan.icon}
                  </div>
                  <h2 className="text-lg font-bold text-stone-100">{plan.name}</h2>
                </div>
                ...
                {plan.features.map(f => (
                  <div key={f.text} className="flex items-start gap-2.5">
                    {f.included
                      ...
                      : <X size={14} className="mt-0.5 flex-shrink-0 text-stone-600" />}
                    <span className={`text-xs leading-snug ${f.included ? 'text-stone-200' : 'text-stone-500'}`}>
                      {f.text}
                    </span>
                  </div>
                ))}
