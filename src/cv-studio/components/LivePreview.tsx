...
function renderOrderedBlock(key: CvMainSectionKey, d: ResumeData, p: ProfileData) {
  switch (key) {
    case 'summary':
      if (!p.summary?.trim())
        return null
            ...
            d.educationItems.map((edu, idx) => (
              <article key={idx} className="mb-2">
                ...
                {edu.description?.trim() ? (
                  <p className="mt-1 whitespace-pre-wrap text-stone-800">{edu.description}</p>
                ) : null}
              </article>
            ))
          )}
    ...
    case 'languages':
      return (
        <Fragment key={key}>
          <h4 className="mt-4 text-xs font-bold uppercase tracking-wide text-stone-800">
            {CvSectionTitleResolver.languagesAndInterests(d)}
          </h4>
          {d.hobbies.length === 0 ? (
            <p className="text-stone-500">Keine Hobbys eingetragen</p>
          ) : (
    ...
    case 'projects':
      ...
      return (
        <Fragment key={key}>
          <h4 className="mt-4 text-xs font-bold uppercase tracking-wide text-stone-800">
            {CvSectionTitleResolver.projects(d)}
          </h4>
          {d.projects.map((proj, idx) => (
            <article key={idx} className="mb-2">
              <strong className="text-stone-900">{proj.name}</strong>
              {proj.description?.trim() ? <p className="mt-1 text-stone-800">{proj.description}</p> : null}
            </article>
          ))}
        </Fragment>
      )
...
export function LivePreview({ resume, pdfDesign }: Props) {
  ...
  return (
    ...
    >
      ...
      {order.map(key => renderOrderedBlock(key, d, p))}
    </div>
  )
}
